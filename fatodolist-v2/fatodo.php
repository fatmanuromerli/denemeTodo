...
try {
    ...
    elseif ($_SERVER['REQUEST_METHOD'] == 'PUT') {
        ...
        $check_id = $data['id'] ?? ''; // Yeni checked değerini al
        $operation = $data['operation'] ?? ''; // Yeni checked değerini al

        if ($operation == 'check_item') {
            $sql = "UPDATE gorevler SET checked = :checked WHERE id=:id"; // Tüm görevleri güncelle
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                'checked' => $checked,
                'id' => $check_id
            ...
            $stmt = $pdo->query("SELECT * FROM gorevler");
            $todos = $stmt->fetchAll();
            echo json_encode($todos); // Tüm görevleri döndür
        } else if (isset($checked)) {
            $sql = "UPDATE gorevler SET checked = :checked"; // Tüm görevleri güncelle
            $stmt = $pdo->prepare($sql);
            $stmt->execute(['checked' => $checked]);
            ...
            echo json_encode(['error' => 'Checked değeri bulunamadı.']);
        }
    }
} catch (PDOException $e) {
